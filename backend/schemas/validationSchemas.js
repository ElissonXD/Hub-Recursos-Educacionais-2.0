...
const aulaValidation = [
    body('titulo').trim().isString().notEmpty().withMessage('Título é obrigatório'),
    ...
    body('conteudos').trim().isArray({ min: 1 }).withMessage('Conteúdos deve conter pelo menos um item'),
    body('recursos').trim().isString().notEmpty().withMessage('Recursos é obrigatório'),
    body('tags').trim().isArray({ min: 1 }).withMessage('Tags deve conter pelo menos uma tag'),
    validate
]
